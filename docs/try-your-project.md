...
The server advertises its review-first constraints during MCP initialization. In `draft-write` mode, the only write tool is `memory_remember`, and it forcibly stores agent-created records as unreviewed even if a caller asks for an active status.
