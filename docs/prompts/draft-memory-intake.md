...
Paste the prompt below into a new Codex task opened in the project you want to evaluate. MiniPMDB must already be configured in `draft-write` mode as described in the [bring-your-own-project guide](../try-your-project.md).
...
1. Confirm that the MiniPMDB tools memory_list, memory_context, memory_audit, and memory_remember are available. If memory_remember is unavailable, explain that MiniPMDB is still in read-only mode and stop without making changes.
