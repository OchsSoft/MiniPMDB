...
Set `MINIPMDB_MCP_MODE=draft-write` to expose `memory_remember`. Here, **draft-write names the permission mode, not the stored lifecycle state**: new memories default to `unreviewed`, and an agent cannot approve its own claim. Review stays an explicit CLI or maintainer action.
